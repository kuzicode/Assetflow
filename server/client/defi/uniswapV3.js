...
export function getAmountsFromLiquidity(sqrtPriceX96, tickLower, tickUpper, liquidity, decimals0, decimals1) {
    const Q96 = 2n ** 96n;
 ...
 */
async function getTokenSymbol(provider, tokenAddr) {
...
async function getTokenDecimals(provider, tokenAddr) {
    ...
    try {
        ...
        try {
            ...
            if (raw && raw !== '0x')
                return Number(ethers.toBigInt(raw));
        }
        catch { }
        return 18;
    }
 ...
 */
export async function fetchUniswapV3Positions(chain, address, provider) {
    const nftAddr = UNISWAP_V3_POSITION_MANAGER[chain];
    const factoryAddr = UNISWAP_V3_FACTORY[chain];
    if (!nftAddr || !factoryAddr)
        return [];
    const positions = [];
    try {
        ...
        for (const tokenId of tokenIds) {
            ...
            try {
                ...
                if (liquidity === 0n && pos.tokensOwed0 === 0n && pos.tokensOwed1 === 0n)
                    continue;
                ...
                if (liquidity > 0n) {
                    ...
                    if (poolAddr !== ethers.ZeroAddress) {
                        ...
                        const amounts = getAmountsFromLiquidity(slot0.sqrtPriceX96, Number(pos.tickLower), Number(pos.tickUpper), liquidity, Number(decimals0), Number(decimals1));
                ...
                let fees1 = 0;
                try {
                    const collected = await withRetry(() => nft.collect.staticCall({ tokenId, recipient: address, amount0Max: MAX_UINT128, amount1Max: MAX_UINT128 }, { from: address }));
