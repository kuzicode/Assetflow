 ...
 */
export async function fetchMorphoVaultApy(vaultAddress = STEAKHOUSE_USDC_BASE, chainId = BASE_CHAIN_ID) {
    ...
    try {
        ...
        const currentPrice = await vault.convertToAssets(unit);
        const now = Date.now();
 ...
 */
export async function fetchMorphoVaultBalances(chain, address, provider, vaultAddresses) {
    const positions = [];
