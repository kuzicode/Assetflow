...
import { fetchOKXTokenBalances, fetchOKXDeFiPositions } from '../defi/okx.js';
...
function getNextEightAmUTC8() {
    const now = Date.now();
    const d = new Date();
    d.setUTCHours(0, 0, 0, 0); // today's 00:00 UTC = today's 08:00 UTC+8
    let next = d.getTime();
    if (next <= now)
        next += 24 * 3600_000; // already passed → use tomorrow
    return next;
}
...
function buildIncomeBreakdown(positions) {
    let uniswap = 0;
    let morpho = 0;
    let hlp = 0;
...
function applyUsdValue(sub, baseToken, prices) {
...
function finalizePositions(groupMap, prices) {
    return Object.entries(groupMap).map(([baseToken, subs]) => {
        for (const sub of subs)
            applyUsdValue(sub, baseToken, prices);
        return {
            baseToken,
            subPositions: subs,
...
export async function buildPositionsSnapshot() {
    ...
    if (okxReady) {
        ...
        for (const wallet of evmWallets) {
            try {
                ...
                for (const token of tokens) {
                    ...
                    if (group === 'STABLE' && !STABLECOIN_SYMBOLS.includes(token.symbol))
                        continue;
                    if (token.symbol && token.amount > 0 && token.usdValue > 0) {
                        ...
                        if (group !== 'STABLE' && group !== token.symbol)
                            putPrice(group, spotPrice);
                    }
                    if (group === 'STABLE')
                        putPrice('STABLE', 1);
                    if (!groupMap[group])
                ...
                await mapWithConcurrency(wallet.chains.filter((chain) => ['ethereum', 'arbitrum', 'optimism', 'base', 'polygon', 'bsc', 'avalanche'].includes(chain)), EXTERNAL_CONCURRENCY, async (chain) => {
                    try {
        ...
        await mapWithConcurrency(wallets, EXTERNAL_CONCURRENCY, async (wallet) => {
            try {
                ...
                for (const balance of balances)
                    allBalances.push({ ...balance, walletLabel: wallet.label });
            }
            catch (error) {
        ...
        const morphoMarketIds = JSON.parse(getSetting('morpho_market_ids') || '[]');
        const morphoVaultAddresses = JSON.parse(getSetting('morpho_vault_addresses') || '[]');
        const defiSubs = [];
        await mapWithConcurrency(wallets, EXTERNAL_CONCURRENCY, async (wallet) => {
            await mapWithConcurrency(wallet.chains, EXTERNAL_CONCURRENCY, async (chain) => {
                ...
                try {
                    ...
                    for (const position of uniPositions) {
                        ...
                        if (position.amount0 > 0.000001)
                            defiSubs.push({ id: uuidv4(), label: `${wallet.label}-${pairLabel}-仓位`, source: 'lp', protocol: 'Uniswap V3', chain, amount: position.amount0, usdValue: 0, _symbol: position.token0Symbol, _group: getBaseTokenGroup(position.token0Symbol) });
                        if (position.amount1 > 0.000001)
                            defiSubs.push({ id: uuidv4(), label: `${wallet.label}-${pairLabel}-仓位`, source: 'lp', protocol: 'Uniswap V3', chain, amount: position.amount1, usdValue: 0, _symbol: position.token1Symbol, _group: getBaseTokenGroup(position.token1Symbol) });
                        if (position.fees0 > 0.000001)
                            defiSubs.push({ id: uuidv4(), label: `${wallet.label}-${pairLabel}-手续费`, source: 'lp_fees', protocol: 'Uniswap V3', chain, amount: position.fees0, usdValue: 0, _symbol: position.token0Symbol, _group: getBaseTokenGroup(position.token0Symbol) });
                        if (position.fees1 > 0.000001)
                            defiSubs.push({ id: uuidv4(), label: `${wallet.label}-${pairLabel}-手续费`, source: 'lp_fees', protocol: 'Uniswap V3', chain, amount: position.fees1, usdValue: 0, _symbol: position.token1Symbol, _group: getBaseTokenGroup(position.token1Symbol) });
                    }
                }
                catch (error) {
                    partialFailureSources.add(`uniswap:${chain}:${wallet.label}`);
                ...
                try {
                    ...
                    for (const position of aavePositions)
                        defiSubs.push({ id: uuidv4(), label: `${wallet.label}-AAVE-${position.symbol}`, source: 'lending', protocol: 'Aave V3', chain, amount: position.amount, usdValue: 0, _symbol: position.symbol, _group: getBaseTokenGroup(position.symbol) });
                }
                catch (error) {
                ...
                try {
                    ...
                    for (const position of morphoPositions)
                        defiSubs.push({ id: uuidv4(), label: `${wallet.label}-Morpho-${position.symbol}`, source: 'lending', protocol: 'Morpho Blue', chain, amount: position.amount, usdValue: 0, _symbol: position.symbol, _group: getBaseTokenGroup(position.symbol) });
                }
                catch (error) {
                ...
                try {
                    ...
                    for (const position of vaultPositions)
                        defiSubs.push({ id: uuidv4(), label: `${wallet.label}-${position.vaultName}`, source: 'lending', protocol: 'Morpho Vault', chain, amount: position.amount, usdValue: 0, _symbol: position.symbol, _group: getBaseTokenGroup(position.symbol) });
                }
                catch (error) {
        ...
        await mapWithConcurrency(wallets, EXTERNAL_CONCURRENCY, async (wallet) => {
            try {
                const hlpPositions = await fetchHyperliquidHlpPositions(wallet.address);
                for (const position of hlpPositions)
                    defiSubs.push({ id: uuidv4(), label: `${wallet.label}-HLP`, source: 'hlp', protocol: 'Hyperliquid HLP', amount: position.equity, usdValue: position.equity, _symbol: position.symbol, _group: getBaseTokenGroup(position.symbol) });
            }
            catch (error) {
                partialFailureSources.add(`hyperliquid:${wallet.label}`);
        ...
        for (const defiSub of defiSubs) {
            if (defiSub._symbol)
                allSymbols.add(defiSub._symbol);
        }
        const priceSnapshot = await fetchPrices([...allSymbols]);
        prices = priceSnapshot.prices;
        for (const symbol of priceSnapshot.missingSymbols)
            missingSymbols.add(symbol);
        for (const source of priceSnapshot.partialFailureSources)
            partialFailureSources.add(source);
        for (const balance of allBalances) {
            const group = getBaseTokenGroup(balance.symbol);
            if (!groupMap[group])
                groupMap[group] = [];
            const price = prices[balance.symbol];
            if (price == null && !STABLECOIN_SYMBOLS.includes(balance.symbol))
                missingSymbols.add(balance.symbol);
            groupMap[group].push({
                id: uuidv4(),
        ...
        for (const defiSub of defiSubs) {
            ...
            if (symbol && price == null && !STABLECOIN_SYMBOLS.includes(symbol))
                missingSymbols.add(symbol);
            const usdValue = symbol && Number.isFinite(price) ? Math.abs(defiSub.amount) * price : defiSub.usdValue;
    ...
    const missingBasePrices = ['ETH', 'BTC', 'BNB'].filter((token) => prices[token] == null);
    if (missingBasePrices.length > 0) {
        const fallbackSnapshot = await fetchPrices(missingBasePrices);
        prices = { ...prices, ...fallbackSnapshot.prices };
        for (const symbol of fallbackSnapshot.missingSymbols)
            missingSymbols.add(symbol);
        for (const source of fallbackSnapshot.partialFailureSources)
            partialFailureSources.add(source);
    }
...
let inFlight = null;
export async function getPositionsSnapshot(options) {
