...
import { deleteRecord, getLatestNonInProgressWeekly, getLatestPnlRecord, getPnlDataDir, getPnlRecordById, insertRecord, listInProgressPnlRecords, listPnlRecords, updateRecord, } from '../repositories/pnlRepo.js';
...
function getIncomeBaselinesPath() {
    return path.join(getPnlDataDir(), 'income_baselines.json');
}
function readRevenueJson() {
    try {
...
function writeIncomeBaselines(data) {
    const p = getIncomeBaselinesPath();
    const tmp = p + '.tmp';
    fs.writeFileSync(tmp, JSON.stringify(data, null, 2), 'utf-8');
    fs.renameSync(tmp, p);
}
function upsertIncomeDailySnapshot(date, values, base) {
...
export function diffDaysUtc(startDate, endDate) {
    const start = new Date(`${startDate}T00:00:00Z`);
    const end = new Date(`${endDate}T00:00:00Z`);
    return Math.max(1, Math.round((end.getTime() - start.getTime()) / 86400000));
}
export function recalcByPnl(startingCapital, pnl, days) {
...
function bakeWeeklyPnlIntoMonthly(settledWeeklyPnl, weekEndDate) {
...
function syncMonthlyPendingFromWeekly(targetEndDate) {
    const monthlyPending = getLatestPnlRecord('monthly', 'pending');
    if (!monthlyPending)
        return;
    ...
    const nextMonthStart = new Date(new Date(`${monthStart}T00:00:00Z`).setUTCMonth(new Date(`${monthStart}T00:00:00Z`).getUTCMonth() + 1))
        .toISOString()
        .slice(0, 10);
    const pendingWeekly = listPnlRecords('weekly').find((r) => r.status === 'pending' && r.startDate >= monthStart && r.startDate < nextMonthStart);
...
export function formatPnlRecord(record) {
    if (!record)
        return null;
    return {
        id: record.id,
...
export function formatRevenue(record) {
    return {
        id: record.id,
...
export async function createWeeklyPnlRecord(input) {
    ...
    const resolvedStartingCapital = input.startingCapital != null
...
export async function createMonthlyPnlRecord(input) {
    ...
    const resolvedEndDate = input.endDate ||
...
export function updatePnlRecordById(id, data) {
...
export function deletePnlRecordById(id) {
...
export async function runDailyPnlAutoAccumulate(today = getTodayUtcDate()) {
    ...
    for (const record of records) {
        if (record.lastAutoUpdateAt === today)
            continue;
...
export function getWeeklyPnlRecords(filters) {
    return listPnlRecords('weekly', filters.from, filters.to).map(formatPnlRecord);
}
export function getMonthlyPnlRecords(filters) {
