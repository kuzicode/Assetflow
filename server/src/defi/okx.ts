...
const CHAIN_INDEX_NAME: Record<string, string> = {
  '1': 'ethereum',
  '56': 'bsc',
  '137': 'polygon',
  '42161': 'arbitrum',
...
function buildHeaders(method: string, path: string, body: string, creds: OKXCredentials) {
...
async function okxGet(pathWithQuery: string, creds: OKXCredentials) {
  ...
  const json = await res.json() as { code: string; msg: string; data: any[] };
  if (json.code !== '0') throw new Error(`OKX error ${json.code}: ${json.msg}`);
  return json.data;
}

async function okxPost(path: string, body: object, creds: OKXCredentials) {
  ...
  if (!res.ok) throw new Error(`OKX API ${res.status}: ${await res.text()}`);
  const json = await res.json() as { code: string | number; msg: string; data: any };
  if (String(json.code) !== '0') throw new Error(`OKX error ${json.code}: ${json.msg}`);
  return json.data;
}
 ...
 */
export async function fetchOKXTokenBalances(address: string, creds: OKXCredentials): Promise<OKXToken[]> {
 ...
 */
export async function fetchOKXDeFiPositions(address: string, creds: OKXCredentials): Promise<OKXDeFiPosition[]> {
  ...
  const positions: OKXDeFiPosition[] = [];
  // data is { walletIdPlatformList: [{ platformList: [...] }] }
  const walletIdPlatformList: any[] = data?.walletIdPlatformList ?? [];
  for (const entry of walletIdPlatformList) {
    for (const platform of (entry.platformList ?? [])) {
      ...
      positions.push({
        protocol: (platform.platformName ?? platform.name) as string,
        chain,
        usdValue,
      });
    }
