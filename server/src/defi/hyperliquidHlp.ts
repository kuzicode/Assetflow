 ...
 */
export async function fetchHlpApy(): Promise<number | null> {
  try {
    ...
    if (typeof data.apr === 'number') return data.apr * 100;

    // Fall back: compute from portfolio value history (30-day trailing annualized)
    const portfolio = data.portfolio as [number, string][] | undefined;
    ...
    const oldEntry = sorted.reduce((prev, curr) =>
      Math.abs(curr[0] - thirtyDaysAgo) < Math.abs(prev[0] - thirtyDaysAgo) ? curr : prev
    );
