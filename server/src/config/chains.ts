...
export function getBaseTokenGroup(symbol: string): string {
  if (STABLECOIN_SYMBOLS.includes(symbol)) return 'STABLE';
  // USD-backed vault tokens (e.g. STEAKUSDC, GTUSDCP, gtUSDC, bbUSDT)
  const upper = symbol.toUpperCase();
  if (upper.includes('USDC') || upper.includes('USDT') || upper.includes('DAI')) return 'STABLE';
  if (['ETH', 'WETH', 'stETH', 'wstETH', 'cbETH', 'rETH'].includes(symbol)) return 'ETH';
  if (['BTC', 'WBTC', 'tBTC', 'cbBTC'].includes(symbol)) return 'BTC';
  if (['BNB', 'WBNB'].includes(symbol)) return 'BNB';
  return symbol; // unknown tokens stay as-is
}

// Popular ERC20 tokens per chain
